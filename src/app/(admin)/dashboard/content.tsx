...
import {
    CustomerProps,
    CustomersDetails,
    MobileCustomersDetails,
} from "@/components/dashboard/CustomerDetails";
...
const tabs: { isBuyer: string }[] = [{ isBuyer: "true" }, { isBuyer: "false" }];
...
export function Dashboard() {
  ...
  const [selectedCustomer, setSelectedCustomer] =
    useState<CustomerProps | null>(customers ? customers[0] : null);
  ...
  return (
    <main className="p-4 sm:px-6 grid grid-cols-1 xl:grid-cols-5 gap-4">
      <div
        className="grid gap-4 h-full xl:col-span-4 "
        ...
        <section className="grid gap-4 grid-cols-1 md:grid-cols-5 grid-rows-1">
          <Card className="md:col-span-3">
            ...
            <CardContent
              ...
              style={{ maxHeight: "calc(100svh - 360px)" }}>
              <Table>
                <TableHeader>
                  <TableRow>
            ...
            <CardContent>
              <RadioGroup
                ...
                <RadioGroupItem
                  ...
                  value="all"
                  key="all"
                  className="border px-4 text-sm py-1 rounded-lg bg-muted/40 transition-colors duration-300 data-[state=checked]:bg-primary data-[state=checked]:text-secondary">
                  Todos
                </RadioGroupItem>
                {tabs.map(({ isBuyer }) => (
                  <RadioGroupItem
                    key={isBuyer}
                    onClick={() => setFilter(isBuyer)}
                    value={isBuyer}
                    className="border px-4 text-sm py-1 rounded-lg bg-muted/40 transition-colors duration-300 data-[state=checked]:bg-primary data-[state=checked]:text-secondary">
                    {buyerStatus[isBuyer]}
                  </RadioGroupItem>
              ...
              {!customers || customers.length === 0 ? (
                ...
                <div
                  ...
                  style={{ maxHeight: "calc(100svh - 285px)" }}>
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {(statusFilter === "all"
                        ...
                        : customers.filter(
                            (customer) =>
                              String(customer.isBuyer) === statusFilter,
                          )
                      ).map((customer) => {
                        ...
                        return (
                          <TableRow
                            ...
                            {window.innerWidth <= 1025 && selectedCustomer && (
                              <MobileCustomersDetails {...selectedCustomer} />
                            )}
                          </TableRow>
                        );
                      })}
        ...
        {customers && customers.length !== 0 ? (
          ...
          <Card className="h-fit col-span-2 lg:col-span-1">
            <CardHeader className="text-start">
              ...
              <CardDescription>
                È aqui que você vera os dados de seus clientes.
              </CardDescription>
            </CardHeader>
          </Card>
        )}
            ...
            data={[
              {
                ...
                count: customers.filter(
                  (customer) => String(customer.isBuyer) === "false",
                ).length,
              ...
              },
              {
                tag: "Clientes",
                count: customers.filter(
                  (customer) => String(customer.isBuyer) === "true",
                ).length,
