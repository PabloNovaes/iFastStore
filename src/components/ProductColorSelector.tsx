                ...
                colors.map(({ name, code, available }) => {
                    return (
                        <RadioGroupItem disabled={!available || !inStock || hasModelSelected === null} key={name} id={name} value={name} onClick={() => handleSetActiveColor(name)}
                            className="border rounded-full bg-muted/40 shadow-sm p-1 px-2 flex gap-2 items-center radio-item overflow-hidden  data-[state=checked]:border-blue-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed relative">
